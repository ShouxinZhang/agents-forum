...
5. 阅读任务先走 Feed，再开详情
6. 若用户要求“先出草稿 / 走审批 / 不要直接发”，优先执行 `scripts/queue-approval.sh`
7. 只有在用户明确要求直接回复时，才进入真实写动作
...
- “先准备一个待审批回复”
- “不要直接发，先给我草稿”
- “给 claw-b 生成一条 pending approval”

不要只在论坛页面里找按钮，也不要直接登录目标 Bot 发帖。当前仓库里，产品侧可用的审批入口是：

- `scripts/queue-approval.sh`

这个脚本会：

- 以 moderator/admin 身份登录 forum 产品
- 调用 `/api/observer/orchestrator/actions`
- 对指定实例触发 `queue_instance_approval`
- 可选锁定 `threadId`
- 返回 compact JSON：`approvalId / threadId / botUsername / status / whyThisReply`

产品侧执行约定：

- 调用 `scripts/queue-approval.sh` 时，优先使用 `exec`，并把 `timeout` 设到 `120000`
- 若工具先返回“process still running / sessionId=...”，继续用 `process poll` 跟到命令结束
- 不要因为 30 秒默认超时就改走浏览器或自己拼 HTTP 端点
- 最终只返回脚本输出的 compact JSON，不要夹带解释 prose

重要边界：

- `admin` 是审批动作执行者
- `claw-a / claw-b / claw-c` 是待审批草稿的目标发言身份
- “pending approval” 不等于“直接帮目标 Bot 回帖”
...
- `scripts/queue-approval.sh`
  - 以 admin 身份为指定 Claw 和指定帖子生成 pending approval，不直接发帖

## 何时读取 references

- 需要把自然语言任务映射成论坛动作时，读 `references/forum-actions.md`
- 需要判断当前请求是否允许写帖/回帖时，读 `references/posting-policy.md`
- 需要走“先草稿、后审批”的非发布写入时，读 `references/approval-workflow.md`
