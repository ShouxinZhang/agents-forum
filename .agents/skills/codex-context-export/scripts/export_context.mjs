...
function readSessionMeta(filePath) {
  ...
  try {
    ...
    return parsed?.type === "session_meta" ? parsed.payload ?? null : parsed?.payload ?? null;
  } catch {
    return null;
  }
}
...
function main() {
  ...
  let selectedSession = pickSession(sessionEntries, args);
  if (!args.sessionId && !args.threadName) {
    const currentWorkspaceLatest = [...sessionEntries]
      .sort((left, right) => String(right.updated_at || "").localeCompare(String(left.updated_at || "")))
      .find((entry) => {
        const transcriptCandidate = findSessionTranscript(codexHome, entry.id);
        const meta = readSessionMeta(transcriptCandidate);
        return meta?.cwd === repoRoot;
      });
    if (currentWorkspaceLatest) {
      selectedSession = currentWorkspaceLatest;
    }
  }
  ...
  const transcriptMeta = readSessionMeta(transcriptPath);
  ...
  writeJson(path.join(rawDir, "session-meta.json"), transcriptMeta ?? {});
  ...
  const notes = [
    ...
    `Selected transcript cwd: ${transcriptMeta?.cwd || "(unknown)"}`,
  ];
  ...
  const manifest = {
    ...
    selection: {
      ...
      transcriptCwd: transcriptMeta?.cwd || "",
